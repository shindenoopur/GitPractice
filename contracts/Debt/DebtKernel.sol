...
import "../TokenRepayment/TermsContract.sol";
 ...
 */
contract DebtKernel is Pausable, ERC721, Ownable {
    ...
    */
    ...
    returns (bool _orderIsValid) {
        ...
        if (ERC721._exists(uint(debtOrder.issuance.agreementId))) {
            emit LogError(
            uint8(Errors.DEBT_ISSUED), debtOrder.debtOrderHash, 
            "from assertDebtOrderValidityInvariants 5");//solhint-disable-line max-line-length
            //solhint-disable-line max-line-length
            return false;
        }
